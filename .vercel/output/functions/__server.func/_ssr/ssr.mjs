...
<html lang="en">
  <head>
    ...
    <style>
      ...
      pre { text-align: left; background: #eee; padding: 1rem; border-radius: 6px; overflow: auto; font-size: 12px; }
  ...
  <body>
    <div class="card">
      ...
      <pre>${err ? err.stack || err.message || JSON.stringify(err) : "No details available"}</pre>
      <div class="actions">
...
async function getServerEntry() {
	if (!serverEntryPromise) serverEntryPromise = import("./server-1z3azbM6.mjs").then((n) => n.o).then((n) => n.t).then((m) => m.default ?? m);
