import { r as createServerFn, t as TSS_SERVER_FUNCTION } from "./server-BBdIZPXp.mjs";
import { a as requireSupabaseAuth, r as cpfParaEmail, t as apenasDigitos } from "./aluno-login-DxsnAriW.mjs";
import { a as object, i as number, n as boolean, o as string, t as array } from "../_libs/zod.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/alunos.functions-CSDYRiKG.js
...
var realizarAdesaoPublica = createServerFn({ method: "POST" }).inputValidator((input) => adesaoSchema.parse(input)).handler(realizarAdesaoPublica_createServerFn_handler, async ({ data }) => {
	...
	const { data: existingAluno, error: getAlunoError } = await supabaseAdmin.from("alunos").select("id, user_id").eq("cpf", cpfLimpo).maybeSingle();
