// ── Contrato PDF Generator ────────────────────────────────
// Ported from contrato-modelo.ts — uses jsPDF via global window.jspdf
...
export function formaPagamentoLabel(id) {
...
/**
 * Generates and downloads the contract PDF using jsPDF (loaded via CDN script tag).
 * @param {{ aluno, contrato, parcelas, texto }} input
 */
export function gerarContratoPdf({ aluno, contrato, parcelas, texto }) {
  // jsPDF is loaded as a global via CDN <script> tag
  ...
  const quebra = (altura = 6) => {
    if (y + altura > 282) { doc.addPage(); y = 16; }
  };
